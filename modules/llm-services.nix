...
in
{
  ...
  config = lib.mkMerge [
    {
      nixpkgs.overlays = [
        # Disable tests for packages with sandbox issues:
        ...
        (final: prev: {
          python3Packages = prev.python3Packages.override {
            overrides = pfinal: pprev: {
              outlines = pprev.outlines.overridePythonAttrs (_: { doCheck = false; });
              rapidocr-onnxruntime = pprev.rapidocr-onnxruntime.overridePythonAttrs (_: { doCheck = false; });
            };
          };
